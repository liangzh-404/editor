...
- `WorkspaceViewModel.importMarkdownPackageToCurrentPage(markdownURL:)` reads the `.md` file, resolves sibling `Attachments/<source-id>/<filename>` paths safely under the package directory, imports each file through `AttachmentRepository`, and preserves the imported attachment block order while copying files into managed attachment storage.
- The normal Markdown file-import UI path now uses the package-aware import method, so a plain `.md` still imports as before and a package export can be imported back with attachment files restored; when no package attachment importer is available, `Attachments/...` links fall back to ordinary paragraph Markdown text instead of creating unavailable attachment blocks.
- Regression coverage: `testImportMarkdownPackageCopiesAttachmentsAndRestoresAttachmentBlocks` and `testPlainMarkdownImportPreservesAttachmentPackageLinksWithoutImporter`; focused Markdown/parser/package regression passed with xcodebuild reporting 0.968s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.799s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
