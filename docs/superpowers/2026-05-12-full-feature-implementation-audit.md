...
| search/backlinks | `search_index` FTS5 table plus `SearchRepositoryTests`/`SearchRepository` cover page title, block text, attachment filename search, and destination page IDs for page/block results; `BacklinkRepositoryTests`/`BacklinkRepository` cover incremental `[[Page]]` backlink maintenance, stale link cleanup, and source page titles; `WorkspaceViewModelTests`/`WorkspaceViewModel` expose search results, selected-page backlinks, search-result selection, and backlink selection; `EditorShellView` renders clickable search results in the page column and clickable backlink rows in the editor | Partial; ranking/snippets, attachment-result navigation, compact-stack deep navigation, and richer link syntaxes remain |
...
| verification | repository/view-model/platform/markdown/sync tests, notebook schema/repository/view-model regressions, page archive regressions, title-editing regressions, page-creation and initial-empty-block focus regressions, native text placeholder regression, sync retry/backoff regressions, remote-fetch merge regressions, app-level manual sync regression, entitlement-gated CloudKit startup regression, search/backlink navigation regressions, 750-block page regression, macOS/iOS app builds, latest macOS app launch evidence, CKException log check, and TextKit availability log check | Partial; macOS UI automation test target exists but is blocked by local system authentication |
