...
- `MarkdownInlineFormatKeyboardResolver` maps exact Cmd-B/Cmd-I/Cmd-Shift-X native text shortcuts to Markdown bold/italic/strikethrough formatting without stealing Cmd-Option or unrelated shortcuts.
...
- Regression coverage: `testMarkdownInlineFormatKeyboardResolverHandlesBoldItalicAndStrikethroughShortcutsOnly`, `testCommandBFormatsSelectionAndKeepsSelectionInEditor`, and `testCommandShiftXFormatsSelectionAndKeepsSelectionInEditor`.
...
1. Continue remaining inline Markdown editing polish that improves everyday writing flow, after the Cmd-B same-format toggle-off and fully selected span toggle-off slices.
