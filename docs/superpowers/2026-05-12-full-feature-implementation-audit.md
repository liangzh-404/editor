...
| attachment blocks | `AttachmentRepositoryTests`, `AttachmentRepository`, `AttachmentThumbnailScheduling`, `AttachmentPreviewState`, `AttachmentPreviewGenerationStatus`, `WorkspaceViewModel.importAttachment`, `CloudKitPrivateDatabaseAdapter`, and `EditorShellView` file importer cover local copy, metadata, attachment block insertion, reload, image/video thumbnail generation and persistence, thumbnail-backed image/video attachment rows, pending preview state/UI for deferred image/video thumbnails, failed preview state/UI with retry scheduling, basic file row rendering, deleting an attachment block reference while retaining attachment metadata/files, explicit unreferenced-attachment GC for metadata/files, deferred thumbnail generation for UI imports, missing-thumbnail generation, background preview scheduling with snapshot refresh, CloudKit original-file `CKAsset` upload/download, CloudKit thumbnail `CKAsset` upload/download, and macOS attachment-toolbar UI automation that imports a fixture through the real toolbar button and verifies the rendered attachment row | Partial; real-account CloudKit validation and broader preview UI automation remain |
...
- Regression coverage: `testAttachmentPreviewStateShowsPendingForMediaWithoutThumbnail`, `testImportImageCanDeferThumbnailAndGenerateItLater`, `testUIAttachmentImportDefersThumbnailAndCanGeneratePreviewLater`, `testUIAttachmentImportSchedulesBackgroundThumbnailGeneration`, `testAttachmentPreviewFailureCanBeRetried`, `testCloudKitPrivateDatabaseAdapterMapsAttachmentAssetsToRecord`, `testCloudKitPrivateDatabaseAdapterDownloadsAttachmentThumbnailAssets`, plus the focused attachment run reporting 1.645s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.079s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
