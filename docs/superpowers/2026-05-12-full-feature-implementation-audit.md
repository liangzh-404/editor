...
- The normal Markdown file-import UI path now uses the package-aware import method, so a plain `.md` still imports as before and a package export can be imported back with attachment files restored.
- Regression coverage: `testImportMarkdownPackageCopiesAttachmentsAndRestoresAttachmentBlocks`; focused Markdown/parser/package regression passed with xcodebuild reporting 1.048s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 9.131s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
