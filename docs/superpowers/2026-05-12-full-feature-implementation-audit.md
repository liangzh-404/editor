...
| block model | `pages` and `blocks` tables plus `PageSummary` / `BlockSnapshot`; paragraph blocks persist; `PageRepository.createPage` creates a new page at the end with an empty editable paragraph and queues page/block sync changes; workspace snapshots now load blocks for all pages so page selection can show the selected page body | Partial; page deletion/archive and folders/notebooks remain |
...
| three-column desktop navigation | `NavigationSplitView` shell, selectable page list, and page-list `+` button for creating and selecting a new editable page | Partial |
| mobile collapsed navigation | compact `NavigationStack` shell and compact page-list `+` button for creating a new editable page | Partial; compact deep navigation after search/backlink selection remains |
...
| verification | repository/view-model/platform/markdown/sync tests, title-editing regressions, page-creation and initial-empty-block focus regressions, native text placeholder regression, sync retry/backoff regressions, remote-fetch merge regressions, app-level manual sync regression, entitlement-gated CloudKit startup regression, search/backlink navigation regressions, 750-block page regression, macOS/iOS app builds, latest macOS app launch evidence, CKException log check, and TextKit availability log check | Partial; macOS UI automation test target exists but is blocked by local system authentication |
...
The next concrete gaps are CloudKit server-change-token persistence plus remote page/workspace/attachment fetch, macOS CloudKit target entitlement once Xcode account/provisioning is available, inline Markdown preservation, page deletion/archive and folder/notebook organization, search ranking/snippet polish, attachment-result navigation, compact-stack deep navigation, and richer advanced block interactions.
