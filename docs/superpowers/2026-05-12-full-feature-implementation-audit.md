...
- The Cmd-K open/update UI flows now follow the block-first launch contract by opening All Documents and selecting Welcome before editing `editor.text.block-welcome-001`. The focused rebuilt run for `testCommandKOpensInlineLinkPanelForSelection` plus `testCommandKUpdatesExistingInlineLinkUnderSelection` passes 2 UI tests with xcodebuild reporting 75.841s and no forbidden SwiftUI runtime issues.

## Recent Inline Link Removal Fix

- `MarkdownInlineLinkRemover` replaces an existing inline Markdown link with its label text and returns that label range for continued editing.
...
- The previously blocked `testCommandKRemovesExistingInlineLinkUnderSelection` UI flow now follows the block-first launch contract by opening All Documents and selecting Welcome before editing the block, passes through the rebuilt `scripts/mac_ui_test.sh run ...` path, and is included in the focused `scripts/block_first_final_regression.sh ui` suite. The refreshed focused UI gate now includes the Cmd-K open/update/remove flows and passes 9 macOS UI tests with xcodebuild reporting 187.890s and no forbidden SwiftUI runtime issues; the xcresult still reports Xcode internal QoS priority-inversion warnings for Cmd-K open/update/removal and Cmd+] promotion tests.
...
- Markdown export capture derives text from the same view-model export path used by the file exporter, so UI automation can verify the visible page Markdown without stopping at the system save panel.
...
## Recent Divider Block Accessibility Polish

- `EditorShellView` now gives divider rows a semantic separator label/value instead of exposing only the raw divider identifier.
- `DividerBlockChromeDescriptor` provides the stable `editor.divider.<blockID>` identifier together with `Divider block` and `Separator` accessibility semantics for future UI automation.
- Regression coverage: `testDividerBlockChromeDescriptorExposesSemanticSeparator` plus the focused divider Markdown/type-change run reporting 0.767s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 7.885s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
...
- `MarkdownTableDocument` parses Markdown table text into editable rows without the separator line, normalizes column counts, and renders normalized rows back to Markdown table syntax.
...
## Recent Structured Table Payload Fix

- `BlockSnapshot` now carries structured `tableRows`, and `PageRepository` stores table cell rows in `payload_json.rows` while keeping Markdown text as the import/export representation.
- `WorkspaceViewModel.updateTableRows` updates table rows through the normal block update path, refreshes derived state, records undo from generated Markdown export text, and restores previous rows during text undo.
- `StructuredTableBlockEditor` edits structured rows directly, falling back to parsing older Markdown-backed table text when loading older blocks.
- Regression coverage: `testTableBlockPersistsStructuredRowsInPayload`, `testUpdateTableRowsPersistsStructuredPayloadAndMarkdownExportText`, `testExportTableBlockUsesStructuredRowsWhenAvailable`, and `testUndoLastTextEditRestoresPreviousStructuredTableRows`; focused table regression passed with xcodebuild reporting 0.911s after the undo regression was added.
- Latest final non-ui regression passed with `scripts/block_first_final_regression.sh non-ui`, including the focused block-first unit suite, EditorMac build, EditorIOS build, and `git diff --check`.
...
## Recent Reference Markdown Import Fix

- `MarkdownTransformer.importBlocks` now recognizes whole-line `[[Page]]` and `[[#Block]]` as typed page-reference and block-reference drafts instead of plain paragraphs.
- `PageRepository.importMarkdown` resolves imported page references by active page title and block references by active block text, stores the target page/block ids in payload JSON, and rebuilds backlinks through the structured reference path.
- Regression coverage: `testImportMarkdownSupportsPageAndBlockReferenceBlocks` and `testImportMarkdownResolvesPageAndBlockReferenceTargets`; focused MarkdownTransformer and PageRepository reference regressions passed with xcodebuild reporting 0.777s and 0.908s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.186s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
...
- The direct text, row-click focus, Return insert/split, Backspace/Forward Delete merge, boundary-arrow focus, and toolbar Add UI flows now follow the block-first launch contract by opening All Documents and selecting Welcome before touching `editor.text.block-welcome-001`. The focused rebuilt run for the latest Backspace/Forward Delete/boundary-arrow/Add group passes 4 UI tests with xcodebuild reporting 100.504s and no forbidden SwiftUI runtime issues; the refreshed `scripts/block_first_final_regression.sh ui` suite now includes the broader core editing set and passes 17 macOS UI tests with xcodebuild reporting 292.595s. The latest xcresult still reports Xcode internal QoS priority-inversion warnings for Backspace, Forward Delete, Cmd-K open/update/removal, Cmd+] promotion, and Return split.
...
## Recent Attachment Identity Matching Fix

- `BlockSnapshot` now carries `attachmentID` for attachment image/video/file blocks, parsed from the block payload's `attachment_id` when repository snapshots load.
- `AttachmentRepository.importAttachment` now returns a block snapshot with the same attachment identity that is persisted into payload JSON.
- `AttachmentSnapshot.matches(block:)` now uses payload attachment identity when available, preventing duplicate filenames with the same attachment kind from binding previews or metadata to the wrong attachment.
- Regression coverage: `testAttachmentSnapshotDoesNotMatchDifferentPayloadAttachmentID` and `testImportedAttachmentBlockCarriesPayloadAttachmentID`; focused attachment regression passed with xcodebuild reporting 2.227s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.862s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Attachment Markdown Export Fix

- `MarkdownTransformer.export` now accepts attachment metadata and renders attachment blocks as stable managed relative links under `Attachments/<attachment-id>/<filename>`, using image syntax for image attachments and normal Markdown links for video/file attachments.
- `WorkspaceViewModel.exportCurrentPageMarkdown` passes the selected page's attachment snapshots into Markdown export, and the shell export button now calls the view-model export closure instead of re-exporting raw visible blocks locally.
- Regression coverage: `testExportAttachmentBlockUsesManagedRelativePathWhenAttachmentMetadataAvailable` and `testExportCurrentPageMarkdownUsesAttachmentRelativePath`; focused Markdown/export/attachment regression passed with xcodebuild reporting 0.929s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.611s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Attachment Markdown Package Export Fix

- `WorkspaceViewModel.exportCurrentPageMarkdownPackage(to:)` now writes the exported Markdown file and copies every visible matched attachment snapshot into a sibling `Attachments/<attachment-id>/<filename>` folder so the managed relative links resolve outside the app container.
- `EditorCanvasView` now calls the package export path from the `fileExporter` success completion for both desktop and compact shells, preserving the existing `FileDocument` picker while adding the attachment-copy side effect after the destination URL is known.
- Regression coverage: `testExportCurrentPageMarkdownPackageWritesMarkdownAndCopiesAttachments`; focused Markdown/export/attachment regression passed with xcodebuild reporting 1.034s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.863s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Attachment Markdown Package Import Fix

- `MarkdownTransformer.importBlocks` now treats whole-line `Attachments/...` Markdown image/file links as attachment drafts while leaving normal inline/external links as text.
- `WorkspaceViewModel.importMarkdownPackageToCurrentPage(markdownURL:)` reads the `.md` file, resolves existing sibling `Attachments/<source-id>/<filename>` files safely under the package directory, imports each file through `AttachmentRepository`, and preserves the imported attachment block order while copying files into managed attachment storage.
- The normal Markdown file-import UI path now uses the package-aware import method, so a plain `.md` still imports as before and a package export can be imported back with attachment files restored; when no package attachment importer or file is available, `Attachments/...` links fall back to ordinary paragraph Markdown text instead of failing the import or creating unavailable attachment blocks.
- `WorkspaceViewModel.markdownImportStatusText` records missing attachment names and `EditorCanvasView` surfaces that status in a compact warning row, then clears the status on the next plain Markdown import.
- Regression coverage: `testImportMarkdownPackageCopiesAttachmentsAndRestoresAttachmentBlocks`, `testPlainMarkdownImportPreservesAttachmentPackageLinksWithoutImporter`, `testImportMarkdownPackagePreservesMissingAttachmentLinksAsText`, and `testPlainMarkdownImportClearsPreviousMarkdownImportStatus`; focused Markdown/parser/package regression passed with xcodebuild reporting 0.967s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.831s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
...
- `WorkspaceViewModel` tracks `AttachmentPreviewGenerationStatus` per attachment so background generation starts as `generating`, failed generation becomes app-visible, retry reschedules the same thumbnail path, and successful retry clears the status after snapshot refresh.
- `AttachmentBlockRow` renders failed preview state with a warning affordance plus an accessible retry button wired back to `WorkspaceViewModel.retryAttachmentPreviewGeneration`.
- `CloudKitPrivateDatabaseAdapter` now includes `asset` and `thumbnailAsset` on uploaded attachment records when the local files exist, and downloads both remote assets into the local attachment directory before applying remote attachment changes.
...
- `AttachmentBlockChromeDescriptor` now gives attachment rows stable kind-aware labels plus preview-ready, generating-preview, and unavailable accessibility values for future UI automation.
- Regression coverage: `testAttachmentPreviewStateShowsPendingForMediaWithoutThumbnail`, `testImportImageCanDeferThumbnailAndGenerateItLater`, `testUIAttachmentImportDefersThumbnailAndCanGeneratePreviewLater`, `testUIAttachmentImportSchedulesBackgroundThumbnailGeneration`, `testAttachmentPreviewFailureCanBeRetried`, `testAttachmentBlockChromeDescriptorExposesKindAndPreviewState`, `testCloudKitPrivateDatabaseAdapterMapsAttachmentAssetsToRecord`, `testCloudKitPrivateDatabaseAdapterDownloadsAttachmentThumbnailAssets`, plus the focused attachment descriptor/preview run reporting 0.887s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.312s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Quote Block UI Polish

- `EditorShellView` now renders quote blocks with visible quote chrome instead of falling back to a plain text row, while keeping the native text editor editable inside the block.
- `QuoteBlockChromeDescriptor` gives quote blocks a semantic accessibility label, value, and stable `editor.quote.<blockID>` identifier for future UI automation.
- Regression coverage: `testQuoteBlockChromeDescriptorExposesSemanticContainer` plus the focused quote Markdown/type-change run reporting 0.769s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.640s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent List Block UI Polish

- `EditorShellView` now renders unordered and ordered list items with visible list markers instead of falling back to a plain text row, while preserving native text editing inside the row.
- `ListBlockOrdinalResolver` numbers ordered list items across contiguous same-parent list runs and skips nested child blocks between numbered siblings.
- `ListBlockChromeDescriptor` gives list blocks semantic accessibility labels, values, stable `editor.unordered-list.<blockID>` / `editor.ordered-list.<blockID>` identifiers, and marker text for future UI automation.
- Regression coverage: `testListBlockChromeDescriptorExposesSemanticContainersAndMarkers` and `testOrderedListOrdinalResolverCountsContiguousSameParentItems`, plus the focused list Markdown/type-change run reporting 0.815s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.806s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Heading Block UI Polish

- `EditorShellView` now wraps H1/H2/H3 native text editors in semantic heading containers instead of relying only on TextKit font size.
- `HeadingBlockChromeDescriptor` gives each heading level a semantic accessibility label, value, stable `editor.heading<level>.<blockID>` identifier, and the SwiftUI header trait for assistive navigation.
- Regression coverage: `testHeadingBlockChromeDescriptorExposesSemanticHeadingLevels` plus the focused heading Markdown/outline/type-change run reporting 0.842s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.723s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
...
## Recent Ordered List Markdown Shortcut Fix

- `MarkdownTransformer.shortcutTransform(for:)` now treats any numeric ordered-list marker shaped like `<number>. ` as an ordered-list shortcut, so continuing a list with `2. ` or `10. ` transforms the current block instead of preserving the marker as paragraph text.
- `WorkspaceViewModel.updateBlockText` uses the broader shortcut path without changing the existing typed-block update/undo flow.
- UIAutomation remains blocked by local macOS authorization: `scripts/block_first_final_regression.sh ui` exited 65 at the doctor because Developer mode is disabled and taskport still requires authentication, and `scripts/block_first_final_regression.sh authorize` timed out after 120s waiting for local approval.
- Regression coverage: `testShortcutTransformsCoreMarkdownMarkers` and `testOrderedListMarkdownShortcutAcceptsContinuingNumbers`; RED captured the previous paragraph fallback for `2. `, GREEN passed with xcodebuild reporting 0.836s, focused Markdown/shortcut regression passed with xcodebuild reporting 1.013s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.766s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Markdown Bullet Marker Compatibility

- `MarkdownTransformer.shortcutTransform(for:)` now accepts `* ` and `+ ` as unordered-list shortcuts, plus `* [ ] ` / `+ [ ] ` and checked variants as task-list shortcuts, matching the parser's broader Markdown marker support.
- `MarkdownTransformer.importBlocks` now parses `*` / `+` unordered items and task items into the same typed block drafts as the existing `-` marker path, while export keeps the current canonical `-` / `- [ ]` / `- [x]` output.
- UIAutomation remains blocked by local macOS authorization: `scripts/block_first_final_regression.sh ui` still exits 65 at the doctor, and a fresh `scripts/block_first_final_regression.sh authorize` attempt timed out after 120s waiting for local approval.
- Regression coverage: `testShortcutTransformsCoreMarkdownMarkers`, `testImportMarkdownIntoBlockDrafts`, `testImportMarkdownSupportsCompletedTaskItems`, and `testAsteriskTaskMarkdownShortcutUpdatesBlockCompletion`; RED captured the previous paragraph fallback for `* [X] `, GREEN passed with xcodebuild reporting 0.839s, focused Markdown/shortcut regression passed with xcodebuild reporting 1.002s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.723s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Fenced Code Import Compatibility

- `MarkdownTransformer.importBlocks` now treats fenced code starts with an info string, such as a backtick fence followed by `swift`, as code-block fences instead of importing the start line and code as paragraph text.
- Closing fences remain explicit plain backtick fences, so the language/info string is ignored for block typing while code contents are preserved in the `codeBlock` text.
- Regression coverage: `testImportMarkdownSupportsFencedCodeInfoString`; RED captured the previous paragraph fallback plus empty code block, GREEN passed with xcodebuild reporting 0.814s. Focused Markdown/import regression passed with xcodebuild reporting 0.864s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.809s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Tilde Fenced Code Import Compatibility

- `MarkdownTransformer.importBlocks` now tracks the opening code fence marker so backtick and tilde fenced blocks close on their matching plain marker.
- Tilde fences with info strings, such as a tilde fence followed by `python`, import into `codeBlock` drafts while preserving only the code contents.
- Regression coverage: `testImportMarkdownSupportsTildeFencedCodeBlocks`; RED captured the previous unsupported tilde fence import, GREEN passed with xcodebuild reporting 0.816s. Focused Markdown/import regression passed with xcodebuild reporting 0.881s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.916s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Setext Heading Markdown Import Compatibility

- `MarkdownTransformer.importBlocks` now recognizes single-line Setext heading bodies followed by `===` or `---` marker lines and imports them as H1/H2 block drafts.
- Standalone `---` still imports as a divider because Setext conversion only runs when exactly one paragraph line is waiting.
- Regression coverage: `testImportMarkdownSupportsSetextHeadings`; RED captured the previous paragraph/divider import, GREEN passed with xcodebuild reporting 0.799s. Focused Markdown/import regression passed with xcodebuild reporting 0.856s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.687s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Outer-Pipe Optional Table Markdown Import Compatibility

- `MarkdownTransformer.importBlocks` now recognizes a pipe-containing header followed by a Markdown table separator row even when rows omit the leading and trailing `|` characters.
- `PageRepository.importMarkdown` persists those imports as `.table` blocks with structured `tableRows` through the existing table payload normalization path.
- UIAutomation was still blocked during that slice by local macOS authorization: `scripts/block_first_final_regression.sh doctor` exited 65 with Developer mode disabled, System Events UI elements disabled, taskport authentication still required, and no automation mode state file; a bounded `EDITOR_UI_TEST_AUTHORIZE_TIMEOUT_SECONDS=30 scripts/block_first_final_regression.sh authorize` attempt timed out waiting for local approval. Later evidence below supersedes that blocker.
- Regression coverage: `testImportMarkdownSupportsTablesWithoutOuterPipes` and `testImportMarkdownPersistsOuterPipeOptionalTableRows`; RED captured the previous paragraph import, GREEN passed with xcodebuild reporting 0.736s. Focused Markdown/import/table regression passed with xcodebuild reporting 3.944s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 9.028s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.

## Recent Block-First Focused UI Pass And Multiline Blockquote Markdown Compatibility

- Local macOS UIAutomation is now unblocked for this focused suite: `scripts/block_first_final_regression.sh doctor` reports Developer mode enabled, System Events UI elements enabled, `_developer` membership present, and the UI Automation preflight ready.
- `testMarkdownExportToolbarCapturesCurrentPageMarkdown` now follows the block-first launch contract by opening All Documents and selecting the Welcome page before asserting/exporting the Welcome block Markdown.
- `MarkdownTransformer.importBlocks` now groups consecutive blockquote lines into a single quote or callout draft, with callout continuation lines preserving the callout block type.
- Markdown export now prefixes every line of multiline quote/callout blocks, preserving Markdown blockquote structure instead of leaving continuation lines as plain paragraphs.
- Regression coverage: `testImportMarkdownSupportsMultilineQuoteAndCalloutBlocks` and `testExportMultilineQuoteAndCalloutBlocksPrefixesEveryLine`; RED captured the previous split quote/callout import and unprefixed export, GREEN passed with xcodebuild reporting 0.851s. Focused Markdown/import/export regression passed with xcodebuild reporting 1.016s. Latest `scripts/block_first_final_regression.sh non-ui` passed with focused unit tests reporting 10.808s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`; latest `scripts/block_first_final_regression.sh ui` passed with the focused macOS UI suite reporting 85.504s.

## Recent Markdown Import Route Preservation And UI Coverage

- A new macOS UI test exposed that page-level Markdown import could return the visible shell to Diary after import, because `WorkspaceViewModel.load()` reapplied the first-launch diary route on every reload when a `DiaryRepository` was present.
- `WorkspaceViewModel.load()` now uses Diary as the first launch collection only once, refreshes the active diary entry on later reloads, and restores the user's previous collection/page selection when the selected page still exists.
- Page-toolbar UI tests now open All Documents and select Welcome before using page-level import/export controls, matching the block-first launch contract instead of assuming the page editor is the first visible route.
- The focused block-first macOS UI gate now includes `testMarkdownImportToolbarRendersAndExportsMultilineQuoteAndCalloutBlocks`, which imports multiline quote/callout Markdown, verifies semantic quote/callout chrome plus native text contents, then exports through the toolbar capture path and checks every blockquote continuation prefix.
- RED evidence: `testImportMarkdownToCurrentPagePreservesPageRouteAfterDiaryLaunch` failed with `selectedCollection` returning `diary`, `selectedPageID` becoming `nil`, and empty visible blocks; the first UI attempt also showed `textViews=editor.diary.text=` after clicking import. GREEN evidence: the focused view-model route/import tests passed with xcodebuild reporting 0.886s, the archive-selection regression check passed with xcodebuild reporting 0.922s, `scripts/block_first_final_regression.sh non-ui` passed with focused unit tests reporting 8.425s plus macOS/iOS builds and `git diff --check`, and the latest `scripts/block_first_final_regression.sh ui` passed with 6 macOS UI tests, 0 failures, xcodebuild reporting 98.792s.

## Recent Large-Page Scroll Peak Metrics Polish

- Baseline before the patch: `scripts/perf_baseline.sh` passed the Release large-page repository check with xcodebuild reporting 4.106s, the Release scroll-metrics baseline with xcodebuild reporting 0.708s, and the Release macOS build, while still emitting the broad `platform=macOS` destination warning.
- RED evidence: the new scroll-metrics unit test first failed because duplicate same-block appearances inflated `blockAppearanceCount` from 2 to 3, `visibleBlockChurnCount` from 3 to 4, and `runtimeSummary` did not expose `peak_last_visible_block_index`.
- `EditorCanvasScrollMetricsTracker` now ignores duplicate same-block/same-index appearances, tracks `peak_last_visible_block_index`, resets that peak on page changes, and exposes it through the DEBUG scroll metrics probe so UI automation can prove a distant lazy block was realized even after the current visible window has moved past it.
- `EditorCanvasView` schedules scroll-metrics `@State` updates onto the next main-queue turn and guards delayed appearance events against the current block list, removing the scroll metrics `Modifying state during view update` warning observed during the stale large-page UI attempt.
- `testLargePageScrollLoadsDistantBlocks` now follows the block-first launch contract by opening All Documents and selecting the seeded Welcome page before reading block 1, then stops scrolling once `peak_last_visible_block_index >= 79` instead of overscrolling to later blocks and requiring block 80 to still be visible.
- GREEN evidence: the focused scroll-metrics unit suite passed with xcodebuild reporting 0.869s; `scripts/mac_ui_test.sh build && scripts/mac_ui_test.sh rerun testLargePageScrollLoadsDistantBlocks` passed with xcodebuild reporting 70.390s; the refreshed `scripts/perf_baseline.sh` uses `platform=macOS,arch=arm64` and passed the Release large-page repository check at 4.525s, Release scroll metrics at 0.720s, and Release macOS build; latest `scripts/block_first_final_regression.sh non-ui` passed with focused unit tests reporting 10.719s, then macOS/iOS builds and `git diff --check`; latest `scripts/block_first_final_regression.sh ui` passed with the focused macOS UI suite reporting 121.081s. At that point, the large-page UI xcresult still reported a separate `Publishing changes from within view updates` warning; the SwiftUI runtime issue gate section below resolves and gates that follow-up.

## Recent SwiftUI Runtime Issue Gate

- RED evidence: `scripts/check_xcresult_runtime_issues.sh` failed against the latest 6-test macOS UI xcresult, reporting `Publishing changes from within view updates` in `testMarkdownExportToolbarCapturesCurrentPageMarkdown` and `testMarkdownImportToolbarRendersAndExportsMultilineQuoteAndCalloutBlocks`.
- Root cause: `PageListView`'s `List(selection:)` binding synchronously called `WorkspaceViewModel.selectPage(id:)` while SwiftUI was applying the list selection update, causing the view model's `@Published` route state to publish during a view update.
- `PageListView` now ignores no-op selections and schedules real page selection onto the next main-queue turn, keeping the visible selection behavior while avoiding synchronous ObservableObject publishing inside SwiftUI's selection transaction.
- `scripts/check_xcresult_runtime_issues.sh` scans an `.xcresult` for actionable SwiftUI runtime issues, and `scripts/mac_ui_test.sh` now runs it after successful `test` / `rerun` actions unless `EDITOR_UI_TEST_SKIP_RUNTIME_ISSUE_CHECK=1` is set.
- GREEN evidence: the two Markdown toolbar UI tests passed through the updated `scripts/mac_ui_test.sh rerun ...` path with xcodebuild reporting 41.671s, followed by `No forbidden SwiftUI runtime issues found` for the generated xcresult; latest `scripts/block_first_final_regression.sh non-ui` passed with focused unit tests reporting 10.513s, then macOS/iOS builds and `git diff --check`; latest `scripts/block_first_final_regression.sh ui` passed with 6 macOS UI tests, 0 failures, xcodebuild reporting 120.613s, and the post-run xcresult check reporting no forbidden SwiftUI runtime issues. The latest 6-test xcresult still reports one Xcode internal QoS priority-inversion warning in `testCommandRightBracketPromotesSelectedDiaryTextToPage`, which remains separate follow-up evidence.

## Recent Diary Startup SwiftUI Runtime Cleanup

- RED evidence: the old `scripts/check_xcresult_runtime_issues.sh` reported `No forbidden SwiftUI runtime issues found` for the focused `testCommandRightBracketPromotesSelectedDiaryTextToPage` xcresult, but exported diagnostics showed five app stderr lines of `[SwiftUI] Modifying state during view update`; after the checker was extended to scan `StandardOutputAndStandardError*.txt`, the same old xcresult failed with those five concrete lines.
- Root cause: macOS `DiaryEditorView` mirrored the AppKit `NSTextView` text through local SwiftUI `@State` (`text` / `syncedEntryID`) during first render and entry refresh. That state mirror was unnecessary on macOS because the source of truth is already the active diary entry plus the AppKit text view's direct change callback.
...
1. Keep the 17-test `scripts/block_first_final_regression.sh ui` suite in the normal final gate now that local UIAutomation is ready.
