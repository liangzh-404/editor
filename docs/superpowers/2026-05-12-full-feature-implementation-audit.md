...
| verification | repository/view-model/platform/markdown/sync tests, notebook schema/repository/view-model regressions, page archive/restore regressions, title-editing regressions, page-creation and initial-empty-block focus regressions, native text placeholder regression, sync retry/backoff regressions, remote-fetch merge regressions, app-level manual sync regression, entitlement-gated CloudKit startup regression, search ranking/snippet/backlink/attachment-result navigation regressions, 750-block page regression, macOS/iOS app builds, latest macOS app launch evidence, CKException log check, and TextKit availability log check | Partial; macOS UI automation test target exists but is blocked by local system authentication |
...
The next concrete gaps are CloudKit server-change-token persistence plus remote notebook/page/workspace/attachment fetch, macOS CloudKit target entitlement once Xcode account/provisioning is available, inline Markdown preservation, richer archive management and nested folder/notebook polish, compact-stack deep navigation, and richer advanced block interactions.
