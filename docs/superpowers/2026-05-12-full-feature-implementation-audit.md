...
- Regression coverage: `testImportMarkdownPackageCopiesAttachmentsAndRestoresAttachmentBlocks`, `testPlainMarkdownImportPreservesAttachmentPackageLinksWithoutImporter`, and `testImportMarkdownPackagePreservesMissingAttachmentLinksAsText`; focused Markdown/parser/package regression passed with xcodebuild reporting 1.027s. Latest `scripts/block_first_final_regression.sh non-ui` also passed with focused unit tests reporting 8.944s, then `EditorMac` build, `EditorIOS` build, and `git diff --check`.
