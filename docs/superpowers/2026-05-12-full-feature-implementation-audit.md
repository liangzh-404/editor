...
- `scripts/mac_ui_test.sh authorize` provides a single local approval entrypoint around `/usr/sbin/DevToolsSecurity -enable`, then reruns `doctor`; `EDITOR_UI_TEST_AUTHORIZE_DRY_RUN=1` prints the command without opening the system authorization prompt.
