...
- UIAutomation remains blocked by local macOS authorization: `scripts/block_first_final_regression.sh doctor` exited 65 with Developer mode disabled, System Events UI elements disabled, taskport authentication still required, and no automation mode state file; a bounded `EDITOR_UI_TEST_AUTHORIZE_TIMEOUT_SECONDS=30 scripts/block_first_final_regression.sh authorize` attempt timed out waiting for local approval.
