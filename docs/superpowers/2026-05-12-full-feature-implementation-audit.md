...
- The direct text, row-click focus, Return insert/split, Backspace/Forward Delete merge, boundary-arrow focus, toolbar Add, toolbar inline-format placeholder, and toolbar inline-link UI flows now follow the block-first launch contract by opening All Documents and selecting Welcome before touching `editor.text.block-welcome-001`. The focused rebuilt run for the latest toolbar format/link group passes 5 UI tests with xcodebuild reporting 100.105s and no forbidden SwiftUI runtime issues; the refreshed `scripts/block_first_final_regression.sh ui` suite now includes the broader core editing and toolbar set and passes 22 macOS UI tests with xcodebuild reporting 369.024s. The latest xcresult still reports Xcode internal QoS priority-inversion warnings for toolbar inline-format/link flows, Backspace, Forward Delete, Cmd-K open/update/removal, Cmd+] promotion, and Return split.
