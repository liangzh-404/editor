...
| search/backlinks | `search_index` FTS5 table plus `SearchRepositoryTests`/`SearchRepository` cover page title, block text, attachment filename search, and destination page IDs for page/block/attachment results; attachment result navigation resolves the owning page through the attachment block payload; `BacklinkRepositoryTests`/`BacklinkRepository` cover incremental `[[Page]]` backlink maintenance, stale link cleanup, and source page titles; `WorkspaceViewModelTests`/`WorkspaceViewModel` expose search results, selected-page backlinks, search-result selection, and backlink selection; `EditorShellView` renders clickable search results in the page column and clickable backlink rows in the editor | Partial; ranking/snippets, compact-stack deep navigation, and richer link syntaxes remain |
...
| verification | repository/view-model/platform/markdown/sync tests, notebook schema/repository/view-model regressions, page archive/restore regressions, title-editing regressions, page-creation and initial-empty-block focus regressions, native text placeholder regression, sync retry/backoff regressions, remote-fetch merge regressions, app-level manual sync regression, entitlement-gated CloudKit startup regression, search/backlink/attachment-result navigation regressions, 750-block page regression, macOS/iOS app builds, latest macOS app launch evidence, CKException log check, and TextKit availability log check | Partial; macOS UI automation test target exists but is blocked by local system authentication |
...
The next concrete gaps are CloudKit server-change-token persistence plus remote notebook/page/workspace/attachment fetch, macOS CloudKit target entitlement once Xcode account/provisioning is available, inline Markdown preservation, richer archive management and nested folder/notebook polish, search ranking/snippet polish, compact-stack deep navigation, and richer advanced block interactions.
