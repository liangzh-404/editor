...
| native protection | `PlatformSecurityTests`, `EditorMac.entitlements`, `DataProtectionService`, `KeychainMetadataStore`, and `CloudKitAccountMetadataService` cover macOS app sandbox, user-selected file read entitlement, native file-protection hook for local database/attachment paths, native Keychain round-trip storage for account/install metadata, and CloudKit account-status metadata persistence through a testable provider boundary | Partial; CloudKit private iCloud scope entitlement and app-visible live account status remain |
...
The next concrete gaps are CloudKit remote fetch/retry/background trigger, CloudKit entitlements, app-visible live account status, table/callout/toggle Markdown coverage, result/backlink navigation polish, and richer advanced block interactions.
