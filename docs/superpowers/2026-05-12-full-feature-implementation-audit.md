...
| attachment blocks | `AttachmentRepositoryTests`, `AttachmentRepository`, `AttachmentThumbnailScheduling`, `AttachmentPreviewState`, `WorkspaceViewModel.importAttachment`, `CloudKitPrivateDatabaseAdapter`, and `EditorShellView` file importer cover local copy, metadata, attachment block insertion, reload, image/video thumbnail generation and persistence, thumbnail-backed image/video attachment rows, pending preview state/UI for deferred image/video thumbnails, basic file row rendering, deleting an attachment block reference while retaining attachment metadata/files, explicit unreferenced-attachment GC for metadata/files, deferred thumbnail generation for UI imports, missing-thumbnail generation, background preview scheduling with snapshot refresh, CloudKit original-file `CKAsset` upload/download, CloudKit thumbnail `CKAsset` upload/download, and macOS attachment-toolbar UI automation that imports a fixture through the real toolbar button and verifies the rendered attachment row | Partial; real-account CloudKit validation and richer preview failure/retry states remain |
