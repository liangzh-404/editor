...
- The direct text, row-click focus, Return insert, and Return split UI flows now follow the block-first launch contract by opening All Documents and selecting Welcome before touching `editor.text.block-welcome-001`. The focused rebuilt run for those four tests passes with xcodebuild reporting 80.286s and no forbidden SwiftUI runtime issues; the refreshed `scripts/block_first_final_regression.sh ui` suite now includes them and passes 13 macOS UI tests with xcodebuild reporting 222.273s. The latest xcresult still reports Xcode internal QoS priority-inversion warnings for Cmd-K open/update/removal, Cmd+] promotion, and Return split.
...
1. Keep the 13-test `scripts/block_first_final_regression.sh ui` suite in the normal final gate now that local UIAutomation is ready.
