...
| block model | `notebooks`, `pages`, and `blocks` tables plus `NotebookSummary`, `PageSummary`, and `BlockSnapshot`; paragraph blocks persist; `PageRepository.createNotebook` creates local Notebook groups; `PageRepository.createPage` creates a new page at the end of the target Notebook with an empty editable paragraph and queues page/block sync changes; `PageRepository.archivePage` hides archived pages and queues a page archive sync change; workspace snapshots now load notebooks and blocks for all visible pages so page selection can show the selected page body | Partial; hard delete, nested folders, notebook rename/reorder, and archive browsing/restore remain |
...
| three-column desktop navigation | `NavigationSplitView` shell, selectable Notebook-grouped page list, Notebook-level `+` buttons for creating and selecting a new editable page in that Notebook, context-menu page archive action, and a New Notebook action | Partial; notebook rename/reorder, nested folders, and archive browsing remain |
| mobile collapsed navigation | compact `NavigationStack` shell, Notebook-grouped page list, Notebook-level `+` buttons for creating a new editable page, context-menu page archive action, and a New Notebook action | Partial; compact deep navigation after search/backlink selection and archive browsing remain |
...
| verification | repository/view-model/platform/markdown/sync tests, notebook schema/repository/view-model regressions, page archive regressions, title-editing regressions, page-creation and initial-empty-block focus regressions, native text placeholder regression, sync retry/backoff regressions, remote-fetch merge regressions, app-level manual sync regression, entitlement-gated CloudKit startup regression, search/backlink/attachment-result navigation regressions, 750-block page regression, macOS/iOS app builds, latest macOS app launch evidence, CKException log check, and TextKit availability log check | Partial; macOS UI automation test target exists but is blocked by local system authentication |
...
The next concrete gaps are CloudKit server-change-token persistence plus remote notebook/page/workspace/attachment fetch, macOS CloudKit target entitlement once Xcode account/provisioning is available, inline Markdown preservation, archive browsing/restore and nested folder/notebook polish, search ranking/snippet polish, compact-stack deep navigation, and richer advanced block interactions.
