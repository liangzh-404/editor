...
| native protection | `PlatformSecurityTests`, `EditorMac.entitlements`, `DataProtectionService`, and `KeychainMetadataStore` cover macOS app sandbox, user-selected file read entitlement, native file-protection hook for local database/attachment paths, and native Keychain round-trip storage for account/install metadata | Partial; CloudKit private iCloud scope entitlement and live iCloud account status integration remain |
...
The next concrete gaps are CloudKit remote fetch/retry/background trigger, CloudKit entitlements and live account validation, table/callout/toggle Markdown coverage, result/backlink navigation polish, and richer advanced block interactions.
