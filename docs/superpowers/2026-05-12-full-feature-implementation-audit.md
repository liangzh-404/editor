...
| CloudKit sync | `SyncRepositoryTests`, `SyncEngineTests`, `SchemaMigratorTests`, `WorkspaceViewModelTests`, `SyncRepository`, `SyncEngine`, `CloudKitSyncAdapter`, `CloudKitRemoteChangeFetching`, and `CloudKitPrivateDatabaseAdapter` cover dirty queue upload through an adapter, local `sync_records` persistence, pending-change clearing, retry metadata in `sync_changes`, failure recording with exponential backoff, skipping deferred changes until `next_attempt_at`, continuing later uploads after one failed change, remote block fetch via a live `CKDatabase.fetch` boundary, applying fetched remote blocks through `SyncMergeEngine`, refreshing visible blocks after manual sync, app-visible manual sync status, and mapping local workspace/notebook/page/block/attachment rows to CloudKit private-database records via a live saver boundary; `EditorCloudKit.entitlements` and `EditorIOS.entitlements` declare the private CloudKit container | Partial; no server-change-token persistence, remote notebook/page/workspace/attachment fetch, push/background scheduling, or macOS target CloudKit entitlement because local Xcode has `No Accounts` for provisioning |
...
| verification | repository/view-model/platform/markdown/sync tests, notebook schema/repository/view-model regressions, page archive/restore regressions, title-editing regressions, page-creation and initial-empty-block focus regressions, native text placeholder regression, row-click editing UI regression coverage, soft-wrapped inline Markdown import regression, sync retry/backoff regressions, remote-fetch merge regressions, app-level manual sync regression, entitlement-gated CloudKit startup regression, search ranking/snippet/backlink/attachment-result navigation regressions, 750-block page regression, macOS/iOS app builds, latest macOS app launch evidence, CKException log check, and TextKit availability log check | Partial; macOS UI automation test target exists but is blocked by local system authentication |
