...
Resume command for local approval: `scripts/block_first_final_regression.sh authorize`, then `scripts/block_first_final_regression.sh ui` for the blocked UI step, or `scripts/block_first_final_regression.sh all` for the whole final sweep. If local approval needs more time, set `EDITOR_UI_TEST_AUTHORIZE_TIMEOUT_SECONDS=<seconds>` on the authorize command. The `all` action refreshes the focused unit suite, app builds, and `git diff --check` before entering the focused macOS UI suite, so while UIAutomation is still blocked it still leaves fresh non-UI evidence before stopping at the local authorization doctor. The runner has already been verified for `help`, `diff-check`, unauthorized `ui` fail-fast, `units`, `builds`, `non-ui`, unauthorized `all` ordering, dry-run `authorize`, and bounded-timeout `authorize`.
